...
#include <utility>
#define WRITEJSON
void FuncAnalysis::ioQualifierCheck() {
...
void FuncAnalysis::IOCheckFuncs(llvm::Instruction *I, llvm::Function *Callee) {
    CallInst *CI = dyn_cast<CallInst>(I);
    if (Ctx->IOFSummaries.find(Callee) == Ctx->IOFSummaries.end() || Ctx->IOFSummaries[Callee].isEmpty()){
        OP<<"no summary.\n";
        return;
    }
            ...
            for (auto item : relatedNode[argNode]) {
                for (auto aa : nAAMap[I][item]) {
                    if (nodeFactory.isArgNode(aa)) {
                        ...
                        OP<<"Related BBs:\n";
                        std::set<std::string> whiteList = Ctx->IOFSummaries[Callee].args[sumArgNode].getIOWhiteList();
                        for (auto item : whiteList) {
                            OP<<item<<":";
                        }
                        OP<<"\n";
			printIORelatedBB(argNode, I, argNo, -1, Callee, whiteList);
                    }
                }
            }
        }
            ...
            for (auto obj : nPtsGraph[I][argNode]) {
                ...
                for (unsigned i = 0; i < checkSize; i++) {
                    if (nodeFactory.isUnOrArrObjNode(obj)) {
                        if (Ctx->IOFSummaries[Callee].reqVec.at(sumObj) == _UNTAINTED) {
                            for (auto item : relatedNode[obj]) {
                                for (auto aa : nAAMap[I][item]) {
                                    if (nodeFactory.isArgNode(aa)) {
                                        ...
                                        OP<<"\n";
                                        printIORelatedBB(obj, I, argNo, -1, Callee, whiteList);
                                    }
                                }       
                            }
                        }
                    ...
                    if (Ctx->IOFSummaries[Callee].reqVec.at(sumObj + i) == _UNTAINTED) {
                        for (auto aa : nAAMap[I][obj+i]) {
                            if (nodeFactory.isArgNode(aa)) {
                                OP<<"Function: "<<F->getName().str()<<"\n";
                                OP << "***********Warning: argument check fails @ argument "
                                    <<argNo<<": variable is tainted, field = "<<i<<"\n";
                                OP<<"Related BBs:\n";
                                std::set<std::string> whiteList = Ctx->IOFSummaries[Callee].args[sumObj+i].getIOWhiteList();
                                for (auto item : whiteList) {
                                    OP<<item<<":";
                                }
                                OP<<"\n";
                                printIORelatedBB(obj+i, I, argNo, i, Callee, whiteList);
                            }
                        }       
                ...
                if (objOffset > 0 && sumObjOffset > 0)
                {
                    for (unsigned i = 0; i < objOffset; i++)
                    {
                        if (Ctx->IOFSummaries[Callee].reqVec.at(sumObj - i) == _UNTAINTED)
                        {
                            OP<<"Function: "<<F->getName().str()<<"\n";
                            OP << "***********Warning: argument check fails @ argument "
                                   <<argNo<<": variable is tainted: field " << objOffset - i<<", nodeIndex = " << obj - i << "\n";
...
void FuncAnalysis::printIORelatedBB(NodeIndex nodeIndex, const llvm::Value *Val,
    ...
    {
        for (auto item : nodeFactory.getIOWL(aa))
        {
    ...
    if (Callee != NULL)
    {
        ...
        for (auto sumObj : Ctx->IOFSummaries[Callee].sumPtsGraph[sumArgNo])
        {
            int sumArgIdx = sumObj + field;
            for (auto witem : Ctx->IOFSummaries[Callee].args[sumArgIdx].getWhiteList())
            {
    ...
    json11::Json jsonObj = json11::Json::object{
        ...
        {"bc", moduleName},
        {"detector", "OOB and IO"},
        {"ioWhitelist", json11::Json(whiteArr)},
    ...
    for (auto item : Ctx->IOFSummaries[F].relatedBC)
    {
        OP << item << ":";
    }
    OP << "\n";
    ...
    if (printWarning == false) {
        //OP<<"insert ("<<bcstr<<" \n";
        //OP<<json_str<<")\n";
        Ctx->fToIOWarns[F].insert(std::make_pair(bcstr, json_str));
        return;
    }
#ifdef WRITEJSON
    std::ofstream jfile;
    OP<<"Write to "<<Ctx->ioJsonFile<<"\n";
    if (Ctx->incAnalysis){
        jfile.open(Ctx->ioJsonIncFile, std::ios::app);
    }
    else {
        jfile.open(Ctx->ioJsonFile, std::ios::app);
    }
