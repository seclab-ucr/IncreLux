...
void setUpOutput(GlobalContext *Ctx, std::string oldDir, std::string newDir) {
		...
		diffMd="/home/yzhai003/inc-experiment/14Mainline/"+Ctx->newVersion+"-"+Ctx->oldVersion+"/md.txt";
		diffFunctions = "/home/yzhai003/inc-experiment/14Mainline/"+Ctx->newVersion+"-"+Ctx->oldVersion+"/bc+func.json";	
		Ctx->newVersion+="_";
		Ctx->oldVersion+="_";
		errs()<< Ctx->absPath<<"\n";
		errs()<<Ctx->outFolder<<"\n";
...
#ifdef REST_F
		/*
		   OP<<"Call Map before REST_F:\n";
		   for (auto i : GlobalCtx.CallMaps) {
		//if (!GlobalCtx.IndirectFuncs.count(i.first->getName().str())) {
		//      continue;
		//}
		OP << i.first->getName().str() << " restcalls: ";
		for (Function *F : i.second) {
		OP << F->getName().str() << "/";
		}
		OP << "\n";
